...
const onSubmit = async (addressData: any) => {
    ...
    try {
        ...
        if (streetNameQuery) {
            const streetNameRequestUrl = `http://server:8000/address/search-address/?query=${encodedStreetName}`;
        ...
        if (sanitizedPostcode) {
            const postcodeRequestUrl = `http://server:8000/address/search-address/?query=${encodedPostcode}`;
